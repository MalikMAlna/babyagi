...
def openai_call(
    ...
    thought_tree.add_thought(prompt)
    while True:
        try:
            ...
            thought_tree.add_thought(response, thought_tree.root)
            return thought_tree.search().task_name
...
class Thought:
    ...
    def is_feasible(self):
        # Simple feasibility check: a thought is feasible if its score is above some threshold
...
class ThoughtTree:
    ...
    def search(self):
        # Simple search algorithm: find the thought with the highest score
        return max(self.root.children, key=lambda thought: thought.score)
...
def evaluation_agent(objective: str, thought: Thought) -> bool:
    ...
    Args:
        objective (str): The objective or goal for the AI to perform the task.
        thought (str): The thought of a task to be evaluated by the AI.

    Returns:
        str: The response score generated by the AI for the given task.
    ...
    # print(context)
    # print('')
    prompt = f'Evaluate one thought for a task based on the following objective: {objective}.\n'
    if context:
        prompt += 'Take into account these previously completed tasks:' + '\n'.join(context)
    prompt += f'\nYour task: {thought.task_name}\nResponse:'
    prompt += f"""\nEvaluate the following task thought with a score between 0.0 (low) to 1.0 (high) based on its potential to contribute to the solution in the context of the current problem state. Provide only the score as a number: 
            {thought.task_name}"""
    thought.score = float(openai_call(prompt)) or 0.0
    print(f"Thought score: {thought.score}")
